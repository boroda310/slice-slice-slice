...
Функция ФайлКонфигурацииСуществует(ПутьКФайлуКонфигурации)
	...
	Если ФайлКонфигурации.Существует() Тогда 
		...
		Пока СтрокаТекстКонфигурации <> Неопределено Цикл
			Если СтрНайти(СтрокаТекстКонфигурации, "ConsoleQueries") > 0 Тогда 	
				Возврат Ложь;
			КонецЕсли;
			СтрокаТекстКонфигурации = ФайлТекстаКонфигурации.ПрочитатьСтроку();
		КонецЦикла;
...
Процедура ВключениеТехнологическогоЖурнала(ПараметрыТехнологическогоЖурнала, РезультатВключения) Экспорт
	...
	Если КаталогКонфигурацииПриложения <> Неопределено Тогда 
		...
		СоздатьКаталог(КаталогСЛогФайлами);
...
//
Процедура ВыключениеТехнологическогоЖурнала(КаталогСЛогФайлами) Экспорт
	...
	Попытка
		УдалитьФайлы(КаталогСЛогФайлами, "*.*");
	Исключение
		ТекстОшибки = НСтр("ru='Не удалось удалить временные файлы технологического журнала. По причине:'")
			+ Символы.ПС + ОписаниеОшибки();
		
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение, ЭтотОбъект,, ТекстОшибки);
	КонецПопытки;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Консоль запросов'", Метаданные.ОсновнойЯзык.КодЯзыка);
